...
def fill_mask(text,label):
    ...
    for idx,(word,tag) in enumerate(result):
        ...
        if (tag in NOUN_CODES) or (tag in ADJECTIVE_CODES):
            new_aug_dict={}
            new_aug_dict['original_sentence']=' '.join(word[0] for word in tmp_result)
            new_aug_dict['masked_word']=tmp_result[idx][0]
            new_aug_dict['masked_position']=idx
            new_aug_dict['label']=label
